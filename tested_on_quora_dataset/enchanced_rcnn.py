...
def enhancedRCNN(num_words = 200000, EMBEDDING_DIM = 300, MAX_SEQUENCE_LENGTH = 60, embedding_matrix = None):
    ...
    M = mean_max(size)
    sentence1_att_mean, sentence1_att_max =  M(c1)
    sentence2_att_mean, sentence2_att_max = M(c2)
    ...
    d1 = Dense(512, activation = 'relu')(subtract)

    #multiply
    subtract = Lambda(lambda x : x[0] * x[1])([sentence1, sentence2])
    d2 = Dense(512, activation = 'relu')(subtract)
